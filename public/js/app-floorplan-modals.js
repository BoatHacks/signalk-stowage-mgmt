...
// Injects sanitized SVG markup into a container and wires up interaction on
// every assignable shape (path/polygon/rect/circle/ellipse with an id).
// Since the SVG comes from an untrusted string (user-uploaded, possibly
// supplied by a third party), we can't express per-shape behavior as Preact
// props — we run it through sanitizeSvg() to strip anything capable of
// executing script, then inject via innerHTML and imperatively attach
// listeners to the resulting DOM, same approach as the original vanilla-JS
// app.
export function FloorplanSvg(props) {
  ...
  useEffect(function () {
    ...
    if (!container) return;
    container.innerHTML = sanitizeSvg(props.svgContent) || '';
