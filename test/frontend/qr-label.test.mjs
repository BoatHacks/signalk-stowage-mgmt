...
test('locationDeepLink: builds the deep link, trimming a trailing slash on the base URL', () => {
  assert.equal(
    ...
    'http://192.168.1.50:3000/plugins/signalk-stowage-mgmt/?location=loc-1'
  )
  assert.equal(
    locationDeepLink('http://192.168.1.50:3000/', 'loc-1'),
    'http://192.168.1.50:3000/plugins/signalk-stowage-mgmt/?location=loc-1'
...
test('locationDeepLink: URI-encodes the location id', () => {
  assert.equal(
    ...
    'http://x/plugins/signalk-stowage-mgmt/?location=loc%20with%20spaces'
  )
})
...
test('buildLabelSvg: produces a scannable SVG QR code with no logo', () => {
  const svg = buildLabelSvg('http://x/plugins/signalk-stowage-mgmt/?location=loc-1')
...
test('buildLabelSvg: different location ids produce different QR data (not a static image)', () => {
  const a = buildLabelSvg('http://x/plugins/signalk-stowage-mgmt/?location=loc-1')
  const b = buildLabelSvg('http://x/plugins/signalk-stowage-mgmt/?location=loc-2')
  assert.notEqual(a, b)
})
